...
!     !  Compute d/dx, d/dy and d/dz of all the cons vars
      ...
      do eq=1,toteq
         ...
         if (if3d) then
            ...
            !added by Kk 03/21 for debug
            !if(nid.eq.15) then
            !   do mm = 1, nxyz1
            !      write(6,'(x,A8,x,I2,x, I2,x, I3,x, ES25.17,x)') 
     >      !       "debug ud", e, eq, mm, ud(mm)
            !   enddo
            !   do mm = 1, nxyz1
            !      write(6,'(x,A8,x,I2,x, I2,x, I3,x, ES25.17,x)') 
     >      !       "debug ur", e, eq, mm, ur(mm)
            !   enddo
            !   do mm = 1, nxyz1
            !      write(6,'(x,A8,x,I2,x, I2,x, I3,x, ES25.17,x)') 
     >      !       "debug us", e, eq, mm, us(mm)
            !   enddo
            !   do mm = 1, nxyz1
            !      write(6,'(x,A8,x,I2,x, I2,x, I3,x, ES25.17,x)') 
     >      !       "debug ut", e, eq, mm, ut(mm)
            !   enddo
            !endif
            !end add
...
c           write(x1, fmt) e
c           write(x2, fmt) eq
c           OPEN(UNIT=12799,FILE='ur.id.'//trim(x1)
c    >       //'.eq.'//trim(x2)//'.afterCompGrad'
c    >       , FORM="FORMATTED", STATUS="REPLACE",ACTION="WRITE")
c              do mm = 1, nxyz1
c                 WRITE(UNIT=12799, FMT = *) "debug ur", e, eq, mm,
c    >               ur(mm)
c              enddo
c           CLOSE(UNIT=12799)
...
c           write(x1, fmt) e
c           write(x2, fmt) eq
c           OPEN(UNIT=12899,FILE='us.id.'//trim(x1)
c    >       //'.eq.'//trim(x2)//'.afterCompGrad'
c    >       , FORM="FORMATTED", STATUS="REPLACE",ACTION="WRITE")
c              do mm = 1, nxyz1
c                 WRITE(UNIT=12899, FMT = *) "debug us", e, eq, mm,
c    >               us(mm)
c              enddo
c           CLOSE(UNIT=12899)
c           endif
     ...
     >                                     sym1(i,1,1,e)*us(i))
      ...
      !if(nid.eq.15) then
      !write(x1, fmt) e
      !OPEN(UNIT=12599+i,FILE='gradu.id.'//trim(x1)
     >! //'.afterCompGrad'
     >! , FORM="FORMATTED", STATUS="REPLACE",ACTION="WRITE")
      !do kk = 1, 3
      !do eq = 1, toteq
      !   do mm = 1, nxyz1
      !      WRITE(UNIT=12599+i, FMT = *) mm, eq, kk, gradu(mm, eq, kk)
      !   enddo
      !enddo
      !enddo
      !CLOSE(UNIT=12599+i)
...
!-----------------------------------------------------------------------
      ...
      if(nid.eq.15) then
         write(6,*) " called set_dealias_face", istep, ilstep
      endif
...
!     single element, but references jacmi and the metrics
      ...
      integer e !, eq !added for debug ud
