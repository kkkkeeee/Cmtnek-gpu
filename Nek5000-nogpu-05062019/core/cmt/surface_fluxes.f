...
!     REAL vf(3)
      ...
      common /nekcb/ cb
      character*3 cb
      character(5) x1, x2 !debug, delete later
...
c           write(x1, '(I4.4)') f
c           write(x2, '(I4.4)') e
c           OPEN(UNIT=9999+i,FILE='flx.f.'//trim(x1)
c    >  //'.e.'//trim(x2),FORM="FORMATTED",
c    >  STATUS="REPLACE",ACTION="WRITE")
c           do j = 1, toteq
c           do mm = 1, nxzd
c             WRITE(UNIT=9999+i, FMT=*) mm, j, flx(mm,j) 
c           enddo
c           enddo
c           CLOSE(UNIT=9999+i)
c        endif
...
c           write(x1, '(I4.4)') f
c           write(x2, '(I4.4)') e
c           OPEN(UNIT=8999+i,FILE='nx.f.'//trim(x1)
c    >  //'.e.'//trim(x2),FORM="FORMATTED",
c    >  STATUS="REPLACE",ACTION="WRITE")
c           do mm = 1, nxzd
c             WRITE(UNIT=8999+i, FMT=*) mm, jaco_f(mm) 
c           enddo
c           CLOSE(UNIT=8999+i)
c        endif
