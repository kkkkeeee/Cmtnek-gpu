...
C> Computes inviscid numerical surface flux from AUSM+ Riemann solver
     ...
     >                         al,tl,rr,ur,vr,wr,pr,ar,tr,flx,cpl,cpr)
      include 'SIZE' !delete later, for debug 02/12
...
!        vf(3) = mfp*wl + mfm*wr
     ...
     >            nm(i)
         !for debug, change back later 01/30 Kk
         !flx(i,1) = rl(i)
         !flx(i,2) = rr(i)
         !flx(i,3) = ul(i)
         !flx(i,4) = ur(i)
         !flx(i,5) = nx(i)
c        if(nid.eq.15 .and. i .eq.1 .and. icalld.eq.0) then
c           write(6,*) "check NAN af:", af, "mfp:", mfp, "rl(i):",rl(i)
c    > ,"rr(i):", rr(i), "nm(i):", nm(i),  "ul(i):", 
c    > ul(i), "ur(i):", ur(i), "pf:", pf, "nx(i):", nx(i), "vl(i):",
c    > vl(i), "vr(i)", vr(i), "ny(i)", ny(i), "wl(i)", wl(i),"wr(i)", 
c    > wr(i), "nz(i)", nz(i), "Hl", Hl, "Hr", Hr, "fs(i)", fs(i)
c           write(6,*) "check detal mf", mf, "al", al(i), "ar", ar(i)
c           icalld = 1
c        endif
      enddo
C> @}
