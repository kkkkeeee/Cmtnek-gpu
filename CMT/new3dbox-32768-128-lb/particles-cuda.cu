...
__global__ void interp_props_part_location(double *rpart, int *ipart, double *vx,double *vy,double *vz,double *t,double *vtrans,double *xgll,double *ygll,double *zgll,double *wxgll,double *wygll,double *wzgll,int n, int nr, int ni, int nx1, int nx1r, int jr, int ju0, int je0, int jtemp, int jrho){
    ...
    if (id < n){
      ...
      double diff;
      double repdum;
      double* bwgtz = new double(nx1);
      double* bwgty = new double(nx1);
      double* bwgtx = new double(nx1);
      double* rep = new double(nx1*nx1*nx1);
      x = rpart[id*nr+jr];
      y = rpart[id*nr+jr+1];
      z = rpart[id*nr+jr+2];
...
__global__ void usr_particles_forces_bdf(double *rpart, int n, int nr, int ndim, int jvol,int jrhop, int jfusr, int jf0,int jrho,int jfqs,int ju0,int jv0, int ja, int jdp, int jre, int jtaup, int jcd, double c_sound){
    int id = blockIdx.x*blockDim.x+threadIdx.x;
    if(id < n){
//          int i = id/ndim;
//          int j = id%ndim;
...
__global__ void usr_particles_forces_rk3(double *rpart, int n, int nr, int ndim, int jvol,int jrhop, int jfusr, int jf0,int jrho,int jfqs,int ju0,int jv0, int ja, int jdp, int jre, int jtaup, int jcd, double c_sound){
    int id = blockIdx.x*blockDim.x+threadIdx.x;
    if(id < n){
//          int i = id/ndim;
//          int j = id%ndim;
          ...
          double uvel[3],vvel[3];
          double c0,T0,mu_p,Re_p,M_p,vel_diff,mu_0=18.27E-6;
          pmass = rpart[id*nr+jvol]*rpart[id*nr+jrhop];
...
//----------
__global__ void update_vel_and_pos_rk3(double *rpart, double *kv_stage_p, double *kx_stage_p, int n, int ndim, int nr, int stage, int fmfac, int jv0,int jv1,int jv2,int jv3,int ju0,int ju1,int ju2,int ju3, int jx0, int jx1, int jx2, int jx3, int jf0){
    int id = blockIdx.x*blockDim.x+threadIdx.x;
...
extern "C" void particles_in_nid_wrapper_(double *rpart, int *ipart, double *rfpts, int *ifpts, int *ifptsmap, double *xerange, int *n, int *nr, int *ni, int *nrf, int *nif, int *nid, int *nfpts, int *lpart, int *nelt, int *jx, int *jy, int *jz,int *je0, int *jrc, int *jpt, int *jd, int *jr){
    ...
    if(inCPU){
        ...
        if(nfpts[0]>0){
            cudaMemcpy(ifptsmap, d_ifptsmap, nfpts[0]*sizeof(int), cudaMemcpyDeviceToHost);
...
//      cudaMemcpy(d_in_part, in_part, n[0]*sizeof(int), cudaMemcpyHostToDevice);
  ...
  update_particle_location<<<gridSize, blockSize>>>(d_rpart,d_xdrange,d_in_part,d_bc_part,n[0],ndim[0],nr[0],jx[0],jx1[0],jx2[0],jx3[0]);
...
extern "C" void interp_props_part_location_wrapper_(double *rpart,int *ipart,double *vx, double *vy, double *vz,double *t, double *vtrans, double *xgll, double *ygll, double *zgll, double *wxgll, double *wygll, double *wzgll, int* n, int* nr, int* ni, int* nx1, int* nx1r, int* jr,int* ju0,int* je0,int* jtemp,int* jrho,int* nelt){
    ...
    double *d_wxgll,*d_wygll,*d_wzgll,*d_xgll,*d_ygll,*d_zgll;
    int *d_ipart;
    if(inCPU){
...
//      cudaMemcpy(d_rep, rep, nx1[0]*nx1[0]*nx1[0]*sizeof(double), cudaMemcpyHostToDevice);
  ...
  int blockSize = 1024, gridSize;
  gridSize = (int)ceil((float)n[0]/blockSize);
  interp_props_part_location<<<gridSize, blockSize>>>(d_rpart,d_ipart,d_vx,d_vy,d_vz,d_t,d_vtrans,d_xgll,d_ygll,d_zgll,d_wxgll,d_wygll,d_wzgll,n[0],nr[0],ni[0],nx1[0],nx1r[0],jr[0],ju0[0],je0[0],jtemp[0],jrho[0]);
  if(inCPU){
      cudaMemcpy(ipart, d_ipart, n[0]*ni[0]*sizeof(int), cudaMemcpyDeviceToHost);
      cudaMemcpy(rpart, d_rpart, n[0]*nr[0]*sizeof(double), cudaMemcpyDeviceToHost);
...
//       cudaFree(d_rep);
 ...
 //---------------------------------------------------------------------------
 extern "C" void usr_particles_forces_bdf_wrapper_(double *rpart, int *n, int* nr, int* ndim, int* jvol,int* jrhop, int* jfusr, int* jf0,int* jrho,int* jfqs,int* ju0,int* jv0, int* ja, int* jdp, int* jre, int* jtaup, int* jcd, double* c_sound){
     ...
     int blockSize = 1024, gridSize;
     gridSize = (int)ceil((float)n[0]/blockSize);
     usr_particles_forces_bdf<<<gridSize, blockSize>>>(d_rpart,n[0],nr[0],ndim[0],jvol[0],jrhop[0],jfusr[0],jf0[0],jrho[0],jfqs[0],ju0[0],jv0[0],ja[0],jdp[0],jre[0],jtaup[0],jcd[0],c_sound[0]);
     if(inCPU){
         cudaMemcpy(rpart, d_rpart, n[0]*nr[0]*sizeof(double), cudaMemcpyDeviceToHost);
         //free
         cudaFree(d_rpart);
...
//---------------------------------------------------------------------------
extern "C" void usr_particles_forces_rk3_wrapper_(double *rpart, int* n, int* nr, int* ndim, int* jvol,int* jrhop, int* jfusr, int* jf0,int* jrho,int* jfqs,int* ju0,int* jv0, int* ja, int* jdp, int* jre, int* jtaup, int* jcd, double* c_sound){
    ...
    int blockSize = 1024, gridSize;
    gridSize = (int)ceil((float)n[0]/blockSize);
    usr_particles_forces_rk3<<<gridSize, blockSize>>>(d_rpart,n[0],nr[0],ndim[0],jvol[0],jrhop[0],jfusr[0],jf0[0],jrho[0],jfqs[0],ju0[0],jv0[0],ja[0],jdp[0],jre[0],jtaup[0],jcd[0],c_sound[0]);
    if(inCPU){
        cudaMemcpy(rpart, d_rpart, n[0]*nr[0]*sizeof(double), cudaMemcpyDeviceToHost);
        //free
        cudaFree(d_rpart);
...
extern "C" void update_vel_and_pos_rk3_wrapper_(double *rpart, double *kv_stage_p, double *kx_stage_p, int* n, int* ndim, int* nr, int* stage, int* fmfac,int* jv0,int* jv1,int* jv2,int* jv3,int* ju0,int* ju1,int* ju2,int* ju3,int* jx0,int* jx1,int* jx2,int* jx3, int* jf0){
    ...
    bool inCPU = false;
    double *d_rpart;
    double *d_kv_stage_p,*d_kx_stage_p;
    if(inCPU){
      cudaMalloc(&d_rpart, n[0]*nr[0]*sizeof(double));
      cudaMemcpy(d_rpart, rpart, n[0]*nr[0]*sizeof(double), cudaMemcpyHostToDevice);
    }
    ...
    int blockSize = 1024, gridSize;
    gridSize = (int)ceil((float)n[0]*ndim[0]/blockSize);
    update_vel_and_pos_rk3<<<gridSize, blockSize>>>(d_rpart, d_kv_stage_p, d_kx_stage_p, n[0], ndim[0], nr[0], stage[0], fmfac[0], jv0[0], jv1[0], jv2[0],jv3[0], ju0[0], ju1[0], ju2[0],ju3[0],jx0[0], jx1[0], jx2[0], jx3[0], jf0[0]);
    if(inCPU){
        cudaMemcpy(rpart, d_rpart, n[0]*nr[0]*sizeof(double), cudaMemcpyDeviceToHost);
        //free
        cudaFree(d_rpart);
