...
c     should we inject particles at this time step?
      print *,"**** start of usr_particles_solver",nid
      ifinject = .false.
...
c     rk3 integration -------------------------------------------------
      elseif (abs(time_integ) .eq. 1) then       
         if (stage.eq.1) then
            print *,"*GPU: u_p_s before update_particle_location",nid
            call update_particle_location   ! move outlier particles
            if (ifinject) call place_particles ! inject particles
            print *,"*GPU: u_p_s before move_particle_location",nid
            call move_particles_inproc      ! update mpi rank
            if (two_way.gt.1) then             ! part. to fluid forcing
            print *,"*GPU: u_p_s before par_solv_near_neigh",nid
               call particles_solver_nearest_neighbor    ! nn
            print *,"*GPU: u_p_s before spread_props_grid",nid, stage
               call spread_props_grid          ! put particle props on grid
             print *,"*GPU: u_p_s after spread_props_grid",nid, stage
            endif
         endif
        print *,"**** GPU: usr_particles_solver interp_props_part_locat",nid
         call interp_props_part_location    ! interpolate
...
c     Other -----------------------------------------------------------
      ...
      print *,"**** End of usr_particles_solver",nid
      return
      end
c----------------------------------------------------------------------
...
c      call MPI_Send(n, 1, MPI_INTEGER, 0, 0, nekcomm, ierr)
...
c     setup files to write to mpi 
      ...
      enddo
      print *,"before mpi_send in line 3943"     
      if(nid.ne.0) then
       call MPI_Send(n, 1, MPI_INTEGER, 0, 0, nekcomm, ierr)
      endif
       npt_total = iglsum(n,1)
